...
def handle_stripe_webhook(payload: dict) -> WebhookResult:
    ...
    Returns 200 on first delivery and also 200 on duplicate (idempotent
    re-delivery). This matches the intended idempotency contract where
    re-delivering the same event is safe and acknowledged with 200.
    """
